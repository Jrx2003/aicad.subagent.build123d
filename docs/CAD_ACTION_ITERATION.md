...
```mermaid
flowchart LR
    U[用户需求 requirements] --> P[Sub-Agent Planner\nCodeGenerator.generate_actions]
    P -->|动作JSON数组| E[Sub-Agent Executor\n_run_aci_attempt]
...
3. `JobExecutor._run_aci_attempt()` 更新本轮状态。
4. 下一轮调用 `CodeGenerator.generate_actions()` 时，将这些字段写入 prompt。
...
sequenceDiagram
    ...
    EX->>CG: generate_actions(requirements, history, suggestions, completeness, previous_error)
    CG-->>EX: actions[]
    loop 每个动作
        EX->>MR: apply_action_sequence([history_actions + new_action])
        MR->>SV: apply_cad_action
...
### 4.3 当前 sub-agent 实际采用的工具路径

虽然 MCP 暴露了 3 个工具，但当前 ACI 主路径主要使用：

1. planner 输出动作 JSON（不是 function-calling）。
2. executor 通过 `apply_action_sequence()` 调用 `apply_cad_action`。
3. `get_history` 目前未进入主循环，只是预留工具，未来可实装，为版本管理提供更多便利。

## 5. 核心模块详细设计

### 5.1 Prompt 层：`codegen_action.md`

目标：把 LLM 限定为“动作规划器”，不是“代码生成器”。

关键约束：

1. 输出只能是 JSON 动作数组或空字符串。
2. 动作字段固定为 `action_type/action_params`。
3. 每轮最多 3 个动作（当前仅测试mvp，不代表最终实现）。
4. 必须利用 `action_history/suggestions/completeness`。

### 5.2 规划层：`sub_agent/codegen.py`

关键实现点：

1. `generate_actions()` 使用 reasoning-tier 模型。
2. prompt 中显式注入 requirements + 当前反馈 + previous_error。
3. `_extract_actions()` 支持纯 JSON、代码块 JSON、降级为空。
4. `_normalize_action()` 统一 `type/params -> action_type/action_params`。
5. 非支持动作会被丢弃并记录日志。

### 5.3 执行编排层：`sub_agent/executor.py`

ACI 循环由 `_run_aci_attempt()` 负责，状态机如下：
