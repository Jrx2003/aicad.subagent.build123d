...
- `RectangleRounded(width, depth, radius=...)` already uses the outer footprint spans. If the requirement gives overall outer width/depth, do not shrink the requested overall width/depth to `width - 2*radius` / `depth - 2*radius` before calling it; only inner cavities or explicit clearances should shrink from the outer envelope.
...
- Do not mix a full `Circle(...)` with helper trim lines plus `make_face()` in the same `BuildSketch` to fake a semicircle or rounded notch profile. Build the arc profile entirely inside `BuildLine`, close it explicitly, then call `make_face()`.
...
- More generally, do not open a nested `BuildPart()` cutter inside an active host and then mutate `host.part -= cutter.part`; keep repeated cuts in the same authoritative host builder with `mode=Mode.SUBTRACT`, or close the host first and subtract one detached positive/private cutter afterward.
...
- For centered clamshell lid/base envelopes, keep the split plane explicit: if the mating plane is `z = split_z`, place the base shell center at `split_z - base_h/2` and the lid shell center at `split_z + lid_h/2`; do not place both shells on the same side of the split plane or let both shells straddle it
- For a centered clamshell with depth on Y, the back seam normally sits at `y = -depth/2` and the front opening boundary at `y = +depth/2`; keep those front/back boundary coordinates literal instead of inferring them from a detached helper center
- Do not swap those front/back seam signs during repair: the hinge belongs on the back seam `y = -depth/2`, while the thumb notch/opening belongs on the front boundary `y = +depth/2`
- For a back-edge pin hinge on a centered clamshell, the seam stays on the back Y boundary while the hinge axis usually runs along X/width. A default `Cylinder(...)` still runs along Z until you rotate the closed solid, for example `Rot(Y=90) * hinge_barrel.part`
...
- `extrude(amount=h)` grows one-sided from the active sketch plane; it does not automatically create a centered `[-h/2, +h/2]` shell interval around that plane.
...
- For that back-edge X-axis hinge lane, keep Y fixed at the seam and derive hinge offsets or spans from width for X placement; do not reuse the seam Y coordinate as an X offset or size that X-axis hinge from depth
- For a front thumb notch on a clamshell, cut inward from the real front shell boundary on a Y-normal host plane such as `Plane.XZ.offset(+depth/2)` or `Plane(face)`; do not center an external notch cutter beyond the shell and hope the overlap is correct
...
- Map named host faces to plane families before opening any local sketch or recess: `top/bottom -> Plane.XY`, `front/back -> Plane.XZ`, and `left/right -> Plane.YZ`. Do not use `Plane.YZ...` for a front-face recess or `Plane.XZ...` for a left-face recess just because the host is centered.
