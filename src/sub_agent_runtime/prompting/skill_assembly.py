...
from sub_agent_runtime.prompting.requirements import (
    _centered_tuple_for_positive_span_axis,
    _detect_named_axis_axisymmetric_pose_mismatch,
    _detect_named_plane_bottom_aligned_box_pose,
    _detect_positive_extrude_bbox_mismatch,
    _detect_positive_extrude_plane,
    _extract_named_axis_axisymmetric_spec,
    _extract_positive_extrude_spec,
    _infer_centered_square_or_rectangular_array_centers,
    _requirement_explicitly_prescribes_revolve_profile,
    _requirement_explicitly_requests_detached_hinge_hardware,
    _requirement_has_explicit_xy_coordinate_pair,
    _requirement_mentions_directional_drilling,
    _requirement_mentions_enclosure_host,
    _requirement_mentions_explicit_path_sweep,
    _requirement_mentions_flange_boss_pattern_holes,
    _requirement_mentions_half_shell_with_split_surface,
    _requirement_mentions_multi_part_assembled_envelope,
    _requirement_mentions_regular_polygon_side_length,
    _requirement_mentions_shelled_host_with_named_face_feature,
    _requirement_prefers_living_hinge,
    _requirement_prefers_named_face_local_feature_sequence,
    _requirement_prefers_nested_regular_polygon_frame,
    _requirement_requests_centered_plane_pose,
    _requirement_suggests_local_finish_probe_family,
    _requirement_suggests_mixed_nested_section,
    _requirement_uses_named_plane_symmetric_union,
    _requirements_text,
)
from sub_agent_runtime.prompting.guidance import (
        ...
        skills.append(
            {
                ...
                "guidance": [
                    ...
                    "For local face edits, use `action_params.face_ref='face:...'` from the latest query_topology candidate sets or matched_ref_ids. Do not fall back to `face='top'`, `face='bottom'`, or `plane='XY'` aliases once exact refs exist.",
                    ...
                    "If the next local edit is already directly expressible on that host face, prefer a direct `apply_cad_action` hole/countersink step with that exact `action_params.face_ref` before opening `create_sketch(face_ref=...)`.",
        ...
        skills.append(
            {
                ...
                "guidance": [
                    ...
                    "On the very next retry, copy one exact `action_params.face_ref='face:...'` from the latest query_topology matched_ref_ids or candidate_sets and keep the local edit bound to that same topology revision.",
        ...
        skills.append(
            {
                ...
                "guidance": [
                    "Candidate-set labels such as `mating_faces`, `outer_faces`, `top_faces`, or `opening_rim_edges` are summaries of multiple refs, not valid `action_params.face_ref` / `action_params.edge_refs` values by themselves.",
                    "On the retry, keep the chosen candidate set only as a selection source, then copy one concrete `face:...` or `edge:...` ref from that candidate set's `ref_ids` into `action_params.face_ref` / `action_params.edge_refs`.",
                    "Do not pass a host-role label or candidate-set id directly into `action_params.face_ref` or `action_params.edge_refs`; use the exact topology ref string from the latest query_topology result.",
