...
class IterativeSubAgentRunner:
    ...
    def __init__(self, app_settings: Settings | None = None) -> None:
        ...
        self._sandbox = McpSandboxRunner(
            ...
            env=self._sandbox_env_from_settings(),
    ...
    def _sandbox_env_from_settings(self) -> dict[str, str]:
        env = os.environ.copy()
        env["LLM_RAPID_PROVIDER"] = self._settings.llm_rapid_provider
        env["LLM_RAPID_MODEL"] = self._settings.llm_rapid_model
        env["LLM_STANDARD_PROVIDER"] = self._settings.llm_standard_provider
        env["LLM_STANDARD_MODEL"] = self._settings.llm_standard_model
        env["LLM_REASONING_PROVIDER"] = self._settings.llm_reasoning_provider
        env["LLM_REASONING_MODEL"] = self._settings.llm_reasoning_model
        return env
