...
def compile_runtime_repair_packet_execution(
    *,
    ...
    if recipe_id in {
        ...
        compiled = _compile_explicit_anchor_hole_repair_packet(
            run_state=run_state,
            packet=packet,
            requirement_text=requirement_text,
        )
...
def _select_runtime_repair_packet(
    ...
    *,
    ...
    return select_preferred_repair_packet(packets, packet_id=packet_id)
...
def _compile_explicit_anchor_hole_repair_packet(
    *,
    run_state: RunState,
    packet: FamilyRepairPacket,
    requirement_text: str,
    ...
    centers_key = (
        ...
        if str(packet.recipe_id or "").strip()
        == "explicit_anchor_hole_centered_host_frame_array"
        else "requested_centers"
    )
    raw_centers = (
        packet.target_anchor_summary.get(centers_key)
        if isinstance(packet.target_anchor_summary, dict)
        else None
    )
    centers = _coerce_xy_points(raw_centers)
    if not centers and centers_key == "requested_centers":
        centers = _coerce_xy_points(parameter_bindings.get("expected_local_centers"))
    if not centers:
        return {
            "ok": False,
            "error": f"repair_packet_missing_{centers_key}",
            "packet": packet.to_dict(),
            "domain_kernel_digest": build_domain_kernel_digest(graph),
        }

    host_face_selector = _host_face_selector_from_packet(packet)
    if host_face_selector != ">Z":
        return {
            "ok": False,
            "error": f"repair_packet_unsupported_host_face:{host_face_selector}",
            "packet": packet.to_dict(),
            "compiled_parameters": {
                "bbox": bbox_xyz,
                "expected_local_centers": centers,
    ...
    hole_diameter = _capture_first_float(
        ...
        (
            r"through-hole\s+diameter\s+([0-9]+(?:\.[0-9]+)?)\s*millimeter",
            r"hole\s+diameter\s+([0-9]+(?:\.[0-9]+)?)\s*millimeter",
            r"([0-9]+(?:\.[0-9]+)?)\s*millimeter[s]?\s+diameter\s+through-hole",
            r"([0-9]+(?:\.[0-9]+)?)\s*millimeter[s]?\s+diameter\s+hole",
        ),
    )
    if hole_diameter is None:
        return {
            "ok": False,
            "error": "repair_packet_missing_hole_diameter",
            "packet": packet.to_dict(),
            "compiled_parameters": {
                "bbox": bbox_xyz,
                "expected_local_centers": centers,
    ...
    hole_call = ""
    if isinstance(packet.recipe_skeleton, dict):
        hole_call = str(packet.recipe_skeleton.get("hole_call") or "").strip().lower()
    wants_countersink = hole_call == "cskhole" or any(
        isinstance(blocker_id, str) and "countersink" in blocker_id
        for blocker_id in feature_instance.blocker_ids
    )
    counter_sink_diameter = None
    counter_sink_angle = None
    counter_sink_depth = None
    if wants_countersink:
        counter_sink_diameter = _capture_first_float(
            str(requirement_text or ""),
            (
                r"head\s+diameter\s+([0-9]+(?:\.[0-9]+)?)\s*millimeter",
                r"counter(?:\s|-)?sink(?:\s+head)?\s+diameter\s+([0-9]+(?:\.[0-9]+)?)\s*millimeter",
            ),
        )
        counter_sink_angle = _capture_first_float(
            str(requirement_text or ""),
            (
                r"cone\s+angle\s+([0-9]+(?:\.[0-9]+)?)\s*degree",
                r"counter(?:\s|-)?sink\s+angle\s+([0-9]+(?:\.[0-9]+)?)\s*degree",
            ),
        )
        if counter_sink_diameter is None:
            return {
                "ok": False,
                "error": "repair_packet_missing_counter_sink_diameter",
                "packet": packet.to_dict(),
                "compiled_parameters": {
                    "bbox": bbox_xyz,
                    "expected_local_centers": centers,
        ...
        if counter_sink_angle is None:
            return {
                "ok": False,
                "error": "repair_packet_missing_counter_sink_angle",
                "packet": packet.to_dict(),
                "compiled_parameters": {
                    "bbox": bbox_xyz,
                    "expected_local_centers": centers,
                    "hole_diameter": hole_diameter,
                    "counter_sink_diameter": counter_sink_diameter,
                    "host_face_selector": host_face_selector,
                },
                "domain_kernel_digest": build_domain_kernel_digest(graph),
            }
        delta_radius = (float(counter_sink_diameter) - float(hole_diameter)) / 2.0
        angle_radians = math.radians(float(counter_sink_angle) / 2.0)
        if delta_radius <= 0.0 or abs(math.tan(angle_radians)) <= 1e-9:
            return {
                "ok": False,
                "error": "repair_packet_invalid_counter_sink_geometry",
                "packet": packet.to_dict(),
                "compiled_parameters": {
                    "bbox": bbox_xyz,
                    "expected_local_centers": centers,
        ...
        counter_sink_depth = round(delta_radius / math.tan(angle_radians), 6)

    hole_centers = [
        (float(center[0]), float(center[1]), float(bbox_xyz[2]) / 2.0)
        for center in centers
    ]
    hole_extent = max(float(bbox_xyz[2]) + 2.0, float(bbox_xyz[2]) * 1.25)
    code_lines = [
        "from build123d import *",
        "",
        f"_aicad_bbox = ({bbox_xyz[0]}, {bbox_xyz[1]}, {bbox_xyz[2]})",
        f"_aicad_hole_diameter = {float(hole_diameter)}",
        f"_aicad_hole_extent = {hole_extent}",
        f"_aicad_hole_centers = {repr(hole_centers)}",
        "with BuildPart() as part:",
        "    Box(*_aicad_bbox)",
        "    with Locations(*_aicad_hole_centers):",
        "        Cylinder(",
        "            _aicad_hole_diameter / 2.0,",
        "            _aicad_hole_extent,",
    ...
    if counter_sink_diameter is not None and counter_sink_depth is not None:
        code_lines.extend(
            [
                f"        Cone(",
                f"            {float(counter_sink_diameter) / 2.0},",
                f"            _aicad_hole_diameter / 2.0,",
                f"            {counter_sink_depth},",
                "            align=(Align.CENTER, Align.CENTER, Align.MAX),",
                "            mode=Mode.SUBTRACT,",
                "        )",
            ]
    ...
    return {
        ...
        "repair_mode": packet.repair_mode,
        "compiler_summary": (
            "Compile the explicit-anchor hole packet into a deterministic centered-host "
            "plate rebuild with one repeated top-face hole array and an optional countersink cone pass."
        ),
        "code": "\n".join(code_lines),
        "packet": packet.to_dict(),
        "compiled_parameters": {
            ...
            "counter_sink_diameter": (
                float(counter_sink_diameter)
                if counter_sink_diameter is not None
                else None
            ),
            "counter_sink_angle": (
                float(counter_sink_angle)
                if counter_sink_angle is not None
                else None
            ),
            "counter_sink_depth": counter_sink_depth,
            "hole_extent": hole_extent,
            "host_face_selector": host_face_selector,
        },
        "domain_kernel_digest": build_domain_kernel_digest(graph),
